...
DBUtils简单说python实现的线程化数据库连接(连接池)，DBUtils支持所有遵循[DP-API 2](https://www.python.org/dev/peps/pep-0249/)规范的数据库连接模块，例如：mysql、sqlserver、oracle、sqlite3等，更多请参考官网：https://cito.github.io/DBUtils/UsersGuide.html。 
...
POOL = PersistentDB(
    ...
    ping=0,  # ping MySQL服务端，检查是否服务可用。# 如：0 = None = never(从不检测), 1 = default = whenever it is requested, 2 = when a cursor is created(创建cursor时候), 4 = when a query is executed, 7 = always
