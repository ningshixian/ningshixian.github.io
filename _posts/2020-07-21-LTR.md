...
> 个性化信息展示背后的技术是排序学习问题（Learning to Rank，LTR）

## 什么是排序学习？

排序学习是机器学习在信息检索系统里的应用，其目标是构建一个排序模型用于对列表进行排序。LTR 是基于特征（利用特征向量来表示文档），通过机器学习算法训练来学习到最佳的拟合公式。
...
可以将LTR归纳为这样一个问题：基于q-d特征向量，通过机器学习算法训练来学习一个最佳的拟合公式，从而对文档打分并排序。

在这个过程中，可以选定学习器（如神经网络、树模型、线性模型等），去自动学习对应的权重应该是多少？学习权重的时候需要确定损失函数，以最小化损失函数为目标进行优化即可得到学习器的相关参数。损失函数的作用相当于告诉学习器应该沿着那个方向去优化，才能保证获得最优的权重。
...
LTR的目标是构造一个相关度函数，按照相关度对于每一个文档进行打分，从而实现对多个条目的排序，这就意味着它的目标值是有结构的。与单值回归和单值分类相比，结构化目标要求解决两个被广泛提起的概念：
...
$$
  Hinge Loss=\min _{W \rightarrow} \sum_{i=1}^{N} \sum_{M_{1}} \max \left(0, s_{j}-y_{n}+1\right)
  $$
...
检索排序：通过点击反馈优化es搜索结果，包括“检索召回”和“精准排序”两个阶段。其中，检索召回阶段负责从候选集中高效快速地召回候选Documents，从而缩减问题规模，降低排序阶段的复杂度，此阶段注重召回算法的效率和召回率；精准排序阶段负责对召回数据进行重排序，采用Learning to Rank相关策略进行排序最优解求解。

![img](https://cdn.zhuanzhi.ai/images/wx/8c9e03bf97d86a6a3ee28dbbfd274334)
...
![](https://ningshixian.github.io/resources/images/排序核心处理流程.png)

大致步骤：

1. Collect Training Data (Queries and their labeled documents)
2. Extract matching **features** from <query, document> pairs
...
基于Light GBM的方案需要特征工程的配合，为此，我们收集了如下几类特征：

![](https://ningshixian.github.io/resources/images/LTR-features.png)



在具体实践中，特征主要包括Statistic Semantic Features（包括F1EXP、F2EXP、TFIDF、BM25等）、Distributed Semantic Features（包括Glove、Doc2vec等）和Ranking Features（召回阶段的排序序列特征），并且这些特征分别从标题、摘要、关键词等多个维度进行抽取，最终构建成特征集合。

![img](https://cdn.zhuanzhi.ai/images/wx/a301a8820d740d1888f77f578a6f6515)



PS：使用GBDT时，不必担心缩放特征值或缺失值（可以保留原样）。 但是，要考虑的一个重要因素是，与线性模型不同，在那些原始计数特征易于快速更改的情况下，将原始计数用作基于树的模型中的特征来进行树遍历决策可能会出现问题。 在这种情况下，最好使用分数比率↓↓↓
...
我们使用在 IR 领域是最先进的**LambdaMART**模型，基于 lightGBM 的 **GBDT** 树模型进行训练，方便dump和load模型，采用 **lambdarank** 损失来进行训练，以 **NDCG** 指标作为优化方向。GBDT 树模型基于 boosting 思想，不断增加决策树，来减小残差。因为不用过多的特征处理，树模型会考虑特征本身的数据分布，同时有很好的学习泛化能力。
